...
f = io.open("COOKIES", "r") -- Read the cookie file.
local cookies
local cookiesOut = {}
if f then
    cookies = f:read("*a")
...
client.rawCallbacks.updateuser:register(function(_, nick)
    ...
    if cookiesOut.cookies then
        local f = io.open("COOKIES", "w") -- Write the cookies to a file.
        f:write(cookiesOut.cookies)
        f:close()
    end
    print("You logged in via ", cookiesOut.action)
end)

client:connect(nick, pass, cookies, cookiesOut) -- Connect to the server.
